...
def populate():
	...
	for m in range(0,male.count()):
		...
		for n in range(0,female.count()):
			...
			point = 0
			if male[m].preReligious != None and (
                male[m].preReligious == female[n].religiousPractice
                or male[m].preReligious == "No Preference"
            ):
				point = (
                    point
                    + match.filter(matchingIndicator="religiousPractice")[0].malePoint
                )
			
			if male[m].preWear != None and (
                havingSameElement(str(male[m].preWear) ,str(female[n].wear)) or male[m].preWear == "No Preference"
            ):
				f.write("\n it works \n")
				point = (
                    point + match.filter(matchingIndicator="wifeWearing")[0].malePoint
                )
			if male[m].preSmoking != None and (
                male[m].preSmoking == female[n].smoker
                or male[m].preSmoking == "No Preference"
            ):
				point = point + match.filter(matchingIndicator="smoking")[0].malePoint
				
			if male[m].preImmigrationStatus != None and (
                havingSameElement(str(male[m].preImmigrationStatus),str(female[n].immigrationStatus))
                or male[m].preImmigrationStatus == "No Preference"
            ):
				point = (
                    point
                    + match.filter(matchingIndicator="immigrationStatus")[0].malePoint
                )
			
			if male[m].preMaritalStatus != None and (
                havingSameElement(str(male[m].preMaritalStatus),str(female[n].maritalStatus))
                or male[m].preMaritalStatus == "No Preference"
            ):
				f.write("\n it works preMaritalStatus\n")
				point = (
                    point + match.filter(matchingIndicator="maritalStatus")[0].malePoint
                )
				
			if male[m].preChildren != None and (
                male[m].preChildren == female[n].children
                or male[m].preChildren == "No Preference"
            ):
				point = point + match.filter(matchingIndicator="children")[0].malePoint
			
			if male[m].preEducation != None and (
                educationStatus(male[m].preEducation) <= educationStatus(female[n].highestEducation)):
				f.write("\n it works preEducation\n")
				point = (
                    point
                    + match.filter(matchingIndicator="educationLevel")[0].malePoint
                )
			if male[m].preEmployment != None and (
                male[m].preEmployment == female[n].employed
                or male[m].preEmployment == "No Preference"
            ):
				point = (
					point + match.filter(matchingIndicator="employement")[0].malePoint
				)
			if (
				male[m].preIncome != None
				and female[n].income != None
				and male[m].preIncome <= female[n].income
			):
				point =point + match.filter(matchingIndicator="minimumIncome")[0].malePoint
				

			if (
				male[m].preAgeGap != None
				and male[m].birthYear != None
				and female[n].birthYear != None
				and male[m].preAgeGap >= (female[n].birthYear -male[m].birthYear)
			):
	...
	for m in range(0,female.count()):
		for n in range(0,male.count()):
			point = 0
			if female[m].preReligious != None and (
				female[m].preReligious == male[n].religiousPractice
				or female[m].preReligious == "No Preference"
			):
				point = (
					point
					+ match.filter(matchingIndicator="religiousPractice")[0].femalePoint
				)
			if female[m].preBeard != None and (
				female[m].preBeard == male[n].beard
				or female[m].preBeard == "No Preference"
			):
				point = point + match.filter(matchingIndicator="beard")[0].femalePoint
			if female[m].preSmoking != None and (
				male[n].smoker or female[m].preSmoking == "No Preference"
			):
				point = point + match.filter(matchingIndicator="smoking")[0].femalePoint
			if female[m].preImmigrationStatus != None and (
				havingSameElement(str(female[m].preImmigrationStatus) , str(male[n].immigrationStatus))
				or female[m].preImmigrationStatus == "No Preference"
			):
				point = (
					point
					+ match.filter(matchingIndicator="immigrationStatus")[0].femalePoint
				)
			if female[m].preMaritalStatus != None and (
				havingSameElement(str(female[m].preMaritalStatus) , str(male[n].maritalStatus))
				
				or female[m].preMaritalStatus == "No Preference"
			):
				point = (
					point
					+ match.filter(matchingIndicator="maritalStatus")[0].femalePoint
				)
			if female[m].preChildren != None and (
				female[m].preChildren == male[n].children
				or female[m].preChildren == "No Preference"
			):
				point = (
					point + match.filter(matchingIndicator="children")[0].femalePoint
				)
			if female[m].preEducation != None and (
				educationStatus(female[m].preEducation) <= educationStatus(male[n].highestEducation)):
				point = (
					point
					+ match.filter(matchingIndicator="educationLevel")[0].femalePoint
				)
			if female[m].preEmployment != None and (
				female[m].preEmployment == male[n].employed
				or female[m].preEmployment == "No Preference"
			):
				point = (
					point + match.filter(matchingIndicator="employement")[0].femalePoint
				)
			if (
				female[m].preIncome != None
				and male[n].income != None
				and female[m].preIncome <= male[n].income
			):
				point = (
					point
					+ match.filter(matchingIndicator="minimumIncome")[0].femalePoint
				)
				
			if (
				female[m].preAgeGap != None
				and female[m].age != None
				and male[n].age != None
				and female[m].preAgeGap >= (female[m].birthYear -male[n].birthYear)
			):
	...
	combined_averages = calculate_element_stats(maleMatrix, femaleMatrix)
	tr_combined_averages = np.transpose(combined_averages)

	for n in range(0,len(female)):
		femalematch = np.array(tr_combined_averages[n])
		femalesorted = np.sort(femalematch)[::-1]
		maleIndices = np.argsort(femalematch)[::-1]

		#f.write("first row \n"+str(malesorted)+"\n"+str(femaleIndices))

		for i in range(0,maxMatchNo):
			try:
				m = int(maleIndices[i])
				checkExist = MatchMaking.objects.filter(maleId=male[m],femaleId=female[n])
				#f.write("checkExist \n"+str(len(checkExist))+"\n")
				if len(checkExist) > 0:
					checkExist.update(percentage=int(femalesorted[i]))
				else:
					new_user = MatchMaking(maleId=male[m], femaleId=female[n], percentage=int(femalesorted[i]),statusReq=0)
					new_user.save()
			except FemaleUser.DoesNotExist:
				f.write(f'nai vai nai')

		for m in range(0,len(male)):
			malematch = np.array(combined_averages[m])
			malesorted = np.sort(malematch)[::-1]
			femaleIndices = np.argsort(malematch)[::-1]

		#f.write("first row \n"+str(malesorted)+"\n"+str(femaleIndices))

		for i in range(0,maxMatchNo):
			try:
				n = int(femaleIndices[i])
				checkExist = MatchMaking.objects.filter(maleId=male[m],femaleId=female[n])
				#f.write("checkExist \n"+str(len(checkExist))+"\n")
				if len(checkExist) > 0:
					checkExist.update(percentage=int(malesorted[i]))
				else:
					new_user = MatchMaking(maleId=male[m], femaleId=female[n], percentage=int(malesorted[i]),statusReq=0)
					new_user.save()
...
# 	for itr in range(0,maxMatchNo):
# 	    top = np.max(malematch)
# 	    ind = np.argmax(malematch)
# 	    malematch[ind]=0
# 	    f.write("first row \n"+str(malematch)+"\n")
    ...
    #Entry in makematching table
    #for m in range(0,male.count()):
    #malematch = np.array(combined_averages[0])
    # top = np.max(malematch)
    # ind = np.argmax(malematch)
    # malematch[ind]=0
    # f.write("1st row \n"+str(malematch)+"\n"+top+"\n"+ind+"\n")
