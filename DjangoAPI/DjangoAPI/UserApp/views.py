...
class AllRequestByMaleView(APIView):
    def get(self, request):
        ...
        for male in requestMales:
            maleId =male.maleId
            female=male.femaleId
            currentUser = MaleUser.objects.get(userId=maleId.userId)
            #statusReq =male.statusReq
            fullName=currentUser.fullName
            email =currentUser.email
            statusOfUser =currentUser.status
            data1 ={"maleId":maleId.userId,"fullName":fullName,"email":email,
                    "status":statusOfUser,"femaleId":female.userId, "femaleName":female.fullName}
    ...
    def get(self, request):
        ...
        for male in requestMales:
            ...
            if currentUser.exists():
                ...
                data1 ={"femaleId":femaleId.userId,"fullName":fullName,"email":email,
                        "status":statusOfUser,"maleId":maleId.userId, "maleName":maleId.fullName}
...
class LockRequestByMaleView(APIView):
    def get(self, request):
        # Get all matchmaking entries for the given maleId
        requestMales = MatchMaking.objects.filter(statusReq=5) | MatchMaking.objects.filter(statusReq=8)
        output =[]
        for male in requestMales:
            matchingId=male.id
            statusReq=male.statusReq
            ...
            email =currentUser.email
            statusOfUser =currentUser.status
            data1 ={"matchingId":matchingId,"statusReq":statusReq,"maleId":maleId.userId,"fullName":fullName,
                    "email":email,"status":statusOfUser,"femaleId":female.userId,"femaleName":female.fullName}
...
class LockRequestByFemaleView(APIView):
    def get(self, request):
        # Get all matchmaking entries for the given maleId
        requestMales = MatchMaking.objects.filter(statusReq=6) | MatchMaking.objects.filter(statusReq=9)
        output =[]
        for male in requestMales:
            matchingId=male.id
            statusReq=male.statusReq
            ...
            if currentUser.exists():
                ...
                email =currentUser.values()[0]['email']
                statusOfUser =currentUser.values()[0]['status']
                data1 ={"matchingId":matchingId,"statusReq":statusReq,"femaleId":femaleId.userId,"fullName":fullName,"email":email,
                        "status":statusOfUser,"maleId":maleId.userId, "maleName":maleId.fullName}
